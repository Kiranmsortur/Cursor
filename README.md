...
3.  **Create a story:** Fill in the form with the story's title, the child's age, the desired tone, and the language.
4.  **Generate the story:** Click the "Generate Story" button. The app will generate a story and an image.
...
3.  **Run the application:**
    ...
    flask run
    ```
    The application will be available at `http://127.0.0.1:5000`. The app will ask for your OpenAI API key in the UI.
...
4.  **Deploy from Vercel:**
    ...
    - No environment variables are needed for the deployment, as the OpenAI API key is provided by the user in the UI.
